...
module.exports = (service, endpoint) => {
  ...
  service.get('/projects', endpoint(async ({ Projects, Forms, Datasets }, { auth, query, queryOptions }) => {
    ...
    let [ projects, forms, datasets ] = await Promise.all([
      Projects.getAllByAuth(auth, queryOptions),
      query.forms && Forms.getAllByAuth(auth, QueryOptions.extended),
      query.datasets && Datasets.getAllByAuth(auth, QueryOptions.extended),
    ]);

    if (query.forms) projects = combineProjectsAndForms(projects, forms);
    if (query.datasets) projects = combineProjectsAndDatasets(projects, datasets);
