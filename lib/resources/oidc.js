...
module.exports = (service, endpoint) => {
  ...
  service.get('/oidc/callback', endpoint(async (container, { Sentry }, req, res) => {
    try {
      ...
      if(!email) {
        Sentry.captureException(new Error(`Required claim not provided in UserInfo Response: 'email'`));
...
function userNotFound(res, email) {
  const message =  `Authentication was successful, but there is no user in the system with the supplied email address (${email}).`;
  log(res.path, 'RETURNING ERROR 401', message);
  // FIXME stop returning error details to client
  return respond(res, 401, page(html`
    <h1>Error!</h1>
...
function errorToFrontend(req, res, errorCode) {
  ...
  return res.redirect(buildTopsyTurvyFrontendUrlFrom(loginUrl));
}
...
function safeNextPathFrom(next) {
  ...
  return buildTopsyTurvyFrontendUrlFrom(url);
}

function buildTopsyTurvyFrontendUrlFrom(url) {
