...
module.exports = (service, endpoint) => {
  ...
  service.get('/oidc/callback', endpoint.html(async (container, _, req, res) => {
    try {
      ...
      console.log({ pkceCodeVerifier, expectedState });
      ...
      console.log({ reqPath:req.url, redirectUri:getRedirectUri(), envDomain, originalUrl:req.originalUrl });
      const requestUrl1 = new URL(req.originalUrl, envDomain);
      const requestUrl2 = new URL(req.originalUrl, getRedirectUri());
      console.log('requestUrl1:', requestUrl1.toString());
      console.log('requestUrl2:', requestUrl2.toString());

      const tokenSet = await authorizationCodeGrant(oidcConfig, requestUrl2, { pkceCodeVerifier, expectedState });
