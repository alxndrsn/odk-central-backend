...
module.exports = (service, endpoint) => {
  ...
  service.get('/oidc/callback', endpoint.html(async (container, _, req, res) => {
    try {
      const pkceCodeVerified = req.cookies[CODE_VERIFIER_COOKIE];
      ...
      console.log({ reqPath:req.url, redirectUri:getRedirectUri() });
      const requestUrl = new URL(req.url, getRedirectUri());
      console.log('requestUrl:', requestUrl);

      const tokenSet = await authorizationCodeGrant(oidcConfig, requestUrl, { pkceCodeVerified, expectedState });
