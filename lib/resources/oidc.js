...
module.exports = (service, endpoint) => {
  ...
  service.get('/oidc/logout', endpoint(async ({ Sessions }, { auth }, req, res) => {
    try {
      const sessionToken = req.cookies['__Host-session'] || req.cookies.session; // TODO should be determined properly
      if (sessionToken) {
        await Sessions.getByBearerToken(sessionToken)
      ...
      const issuerUrl = config.get('default.oidc.issuerUrl');

      if (client.post_logout_redirect_uris.length) {
        const logoutUrl = client.endSessionUrl({
          // TODO can we remove this arg completely?
        });
        res.redirect(logoutUrl); // looks like this would break some expectation of endpoint() managing headers
      } else if (issuerUrl.endsWith('.auth0.com')) {
        // auth0 implementation doesn't send end_session_endpoint, even though it exists.
        // Their docs claim it is expected, but the documented example does not actually return it:
        //
        //     $ curl https://acme.eu.auth0.com/.well-known/openid-configuration | jq .end_session_endpoint
        //
        // See: https://auth0.com/docs/authenticate/login/logout/log-users-out-of-auth0
        //
        // REVIEW: do we want to support special cases like this? FIXME
        const client_id = config.get('default.oidc.clientId');
        const post_logout_redirect_uri = getPostLogoutRedirectUri();

        //res.redirect(issuerUrl + '/v2/logout?' + new URLSearchParams({ client_id, returnTo }));
        res.redirect(issuerUrl + '/oidc/logout?' + new URLSearchParams({ client_id, post_logout_redirect_uri }));
      } else {
        // no support for rp-initialised logout, so just go home
        res.redirect('/');
      }
    } catch(err) {
      // TODO remove logging
      console.log('/oidc/logout', 'threw:', err);
  ...
  service.get('/oidc/login', async (req, res) => {
    try {
      ...
      // TODO remove logging
      console.log('/oidc/login', 'threw:', err);
      return serverError(res, err);
    }
  });
  ...
  service.get('/oidc/callback', endpoint(async (container, {}, req, res) => {
    try {
      ...
      if(!email_verified) return errorToFrontend(req, res, 'email-not-verified');
      ...
      console.log('oidc/callback threw unexpected error:', err);
