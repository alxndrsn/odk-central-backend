...
module.exports = (service, endpoint) => {
  service.get('/oidc/1', async (req, res, next) => {
    try {
      ...
      const authUrl = client.authorizationUrl({
        ...
        resource: `${myConfig.centralRootUrl}/v1`,
        code_challenge,
  ...
  service.get('/oidc/2', async (req, res, next) => {
    try {
      ...
      if(!myConfig.allowUnverifiedEmail) {
...
function getRedirectUri() {
  return `${myConfig.centralRootUrl}/v1/oidc/2`;
}

const myConfig = {
  discoveryUrl: 'https://accounts.google.com',
  centralRootUrl: 'http://localhost:8989',
  client_id: '564021877275-o5q3i8j44190d93d9mldd3rti1fncn3u.apps.googleusercontent.com', // TODO this should come from config,
  client_secret: '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', // TODO this should come from env var!
};

const clientLoader = (async () => {
  const issuer = await Issuer.discover(myConfig.discoveryUrl);
