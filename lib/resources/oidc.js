...
module.exports = (service, endpoint) => {
  ...
  service.get('/oidc/callback', endpoint(async (container, _, req, res) => {
    try {
...
function debugIdpResponse(res, session, userinfo, user) {
  ...
  return respond(res, page(html`
    ...
    <div><h3>User     </h3><code><pre>${JSON.stringify(user,     null, 2) /* eslint-disable-line no-multi-spaces */}</pre></code></div>
    <div><h3>Session  </h3><code><pre>${JSON.stringify(session,  null, 2) /* eslint-disable-line no-multi-spaces */}</pre></code></div>
    <div><h3>User Info</h3><code><pre>${JSON.stringify(userinfo, null, 2) /* eslint-disable-line no-multi-spaces */}</pre></code></div>
    <div>
      <h2>TODO</h2>
      <pre>
