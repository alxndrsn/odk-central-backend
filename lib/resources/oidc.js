...
// FIXME use __Secure- in production.  But not in dev - even though firefox will support __Secure with localhost, chrome will not.  Note that this behaviour is similar but distinct from the Secure attribute, which seems to send cookies to http://localhost on both Chrome and FireFox.
// See:
...
module.exports = (service, endpoint) => {
  ...
  service.get('/oidc/login', async (req, res) => {
    try {
      ...
      // TODO confirm Lax is correct once all other settings are locked down!
      res.cookie(CODE_VERIFIER_COOKIE, code_verifier, {
        httpOnly: true,
  ...
  service.get('/oidc/callback', endpoint(async (container, {}, req, res) => {
    try {
      ...
      // This could probably be checked in claims_supported when discovering the issuer.
      // TODO remove this config option, and configure automatically?
      ...
      return respond(res, page(
        ...
        html`
          ...
          <div><a href="/" id="cl">Continue to ODK Central</a></div>
        `,
      ));
    } catch(err) {
