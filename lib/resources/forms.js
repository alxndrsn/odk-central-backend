...
module.exports = (service, endpoint) => {
  ...
  service.get('/projects/:projectId/formList', endpoint.openRosa(({ Forms, Projects, env }, { auth, params, originalUrl, queryOptions }, req) =>
    Projects.getById(params.projectId)
      ...
      .then((project) => Forms.getByAuthForOpenRosa(project.id, auth, queryOptions.allowArgs('formID'), req.headers['user-agent']?.includes('Enketo')))
      .then((forms) => formList({ forms, basePath: path.resolve(originalUrl, '..'), domain: env.domain })))); // XXX
