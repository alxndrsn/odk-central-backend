...
module.exports = (service, endpoint) => {
  ...
  service.get('/projects/:projectId/formList', endpoint.openRosa(({ Forms, Projects, env }, { auth, params, originalUrl, queryOptions }, req) =>
    console.log('/formList',
        req.headers,
        req.headers['x-enketo-referrer-url'],
        req.headers['x-enketo-referrer-url']?.match('/-/(edit|preview)/')
    ) || Projects.getById(params.projectId)
      .then(getOrNotFound)
      .then(rejectIf((() => auth.actor.isEmpty()), noargs(Problem.user.openRosaAuthenticationRequired)))
      .then((project) => Forms.getByAuthForOpenRosa(project.id, auth, queryOptions.allowArgs('formID'), req.headers['x-enketo-referrer-url']?.match('/-/(edit|preview)/')))
  ...
  formResource('/projects/:projectId/forms/:id', (Forms, params, withXml = false, options = QueryOptions.none) =>
    console.log('Fetching form:', params.projectId, params.id) || Forms.getByProjectAndXmlFormId(params.projectId, params.id, withXml, null, options)
      .then(getOrNotFound));
