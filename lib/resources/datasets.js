...
const csvHeaderProps = [
  { expr:sql`entities.uuid`, alias:"__id" },
  { expr:sql`entity_defs.label`, alias:'label' },
];
const csvSystemProps = [
  ...
  { expr:sql`to_char(entities."createdAt" AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"')`, alias:'__createdAt' },
  { name:'entities.creatorId',               alias:'__creatorId' },
  { name:'actors.displayName', alias:'__creatorName' },
  { expr:sql`entity_defs.version-1`,                 alias:'__updates' },
  { name:'entities.updatedAt',               alias:'__updatedAt' },
  { name:'entity_defs.version',             alias:'__version' },
];
...
module.exports = (service, endpoint) => {
  ...
  service.get('/projects/:projectId/datasets/:name/entities.csv', endpoint(async ({ Auth, Actors, Datasets, Entities, Projects }, { params, auth, query }, request, response) => {
    ...
    const csv = async () => {
      ...
      return Entities.streamForExport(dataset.id, [ ...csvHeaderProps, ...properties, ...csvSystemProps ], options, filterActorId);
    };
  ...
  service.get('/projects/:projectId/trash/datasets/:datasetId/entities.csv', endpoint(async ({ Datasets, Entities, Projects }, { params, auth }, request, response) => {
    ...
    response.set('Content-Type', 'text/csv');
    return Entities.streamForExport(dataset.id, properties, QueryOptions.extended);
  }));
