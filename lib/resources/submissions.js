...
module.exports = (service, endpoint) => {
  ...
  const openRosaSubmission = (path, draft, getForm) => {
    ...
    // Nonstandard REST; OpenRosa-specific API.
    service.post(path, multipart.any(), endpoint.openRosa(({ Forms, Submissions, SubmissionAttachments }, { params, files, auth, query, headers }, req) =>
      Submission.fromXml(findMultipart(files).buffer)
        .then((partial) => getForm(auth, params, partial.xmlFormId, Forms, partial.def.version, req)
  ...
  openRosaSubmission('/projects/:projectId/submission', false, (auth, { projectId }, xmlFormId, Forms, version, req) =>
    Forms.getByProjectAndXmlFormId(projectId, xmlFormId, false, version)
      .then(getOrNotFound)
      .then(ensureDef)
      .then(rejectIf(((form) => !form.acceptsSubmissions() && !req.headers['user-agent']?.startsWith('Enketo')), noargs(Problem.user.notAcceptingSubmissions)))
