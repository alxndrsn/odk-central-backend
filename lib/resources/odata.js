...
module.exports = (service, endpoint) => {
  ...
  const odataResource = (base, draft, getForm) => {
    ...
    service.get(`${base}/:table`, endpoint.odata.json(async ({ Forms, Submissions, env }, { auth, params, originalUrl, query }) => {
      ...
      const [fields, stream, count] = await Promise.all([
      ...
      const odata = rowStreamToOData(fields, params.table, env.domain, originalUrl, query, stream, count);
      console.log('odata:', odata);
      console.log('Gonna JSON...');
      return json(odata);
    }));
  };
