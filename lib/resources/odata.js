    ...
    const singleRecord = endpoint.odata.json(({ Forms, Submissions, env }, { auth, params, query, originalUrl }) =>
      getForm(Forms, auth, params)
        .then((form) => Promise.all([
          Forms.getFields(form.def.id).then(async allFields => {
            const table = getTableFromOriginalUrl(originalUrl);
            const fieldSelecter = selectFields(query, table);
            return fieldSelecter(allFields);
          }),
    ...
    service.get(`${base}/:table`, endpoint.odata.json(({ Forms, Submissions, env }, { auth, params, originalUrl, query }) =>
      getForm(Forms, auth, params)
        .then(async (form) => {
          ...
          const selectedFields = await selectFields(query, params.table)(allFields);

          const stream = await Submissions.streamForExport(form.id, draft, undefined, options);

          const { count, remaining } = await ((params.table === 'Submissions') && options.hasPaging())
              ? Submissions.countByFormId(form.id, draft, options) : resolve({});

          return json(rowStreamToOData(selectedFields, params.table, env.domain, originalUrl, query, stream, count, remaining));
        })));
  };
