  ...
  if(oidc.isEnabled()) {
    ...
    service.post('/users', endpoint(({ Users, mail }, { body, auth }) =>
      auth.canOrReject('user.create', User.species)
        ...
        .then(Users.create)
        // REVIEW: is this considered an acceptable way of blocking password-based login?
        .then((savedUser) => Users.updatePassword(savedUser, randomPassword())
            .then(() => mail(savedUser.email, 'accountCreatedForOidc'))
      ...
      .then((user) => auth.canOrReject('user.update', user.actor)
        .then(() => {
          if (oidc.isEnabled()) {
            ...
            // REVIEW is there some official, lower-level place to do this, e.g. in lib/model/frames.js?
        ...
        .then((patchData) => console.log('Updating user with data:', patchData) || Users.update(user, patchData)
...
// Generate an "unguessable" password to effectively prevent an account from
