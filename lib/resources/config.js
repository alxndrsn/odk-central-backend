...
const sleep = (id, ms) => console.log(`[${id}]`, 'Sleeping for', ms, 'ms...') || new Promise(resolve => setTimeout(() => console.log(`[${id}]`, 'Woke up!') || resolve(), ms));
...
module.exports = (service, endpoint) => {
  ...
  // BACKUPS
  //
  service.post('/tx-clash/wait-before-update', endpoint(async ({ Configs, db }, { auth }) => {
    const thingsOriginal = await db.query(sql`SELECT * FROM some_thing`); // suffix FOR UPDATE if we want this to work
    console.log(`[wait-before-update]`, 'SELECT 1 complete.')
    const originalCounter = thingsOriginal.rows[0].counter;

    await sleep('wait-before-update', 10000);

    const expectedFinalCounter = originalCounter + 5;
    await db.query(sql`UPDATE some_thing SET counter=counter+5`); // no WHERE clause as (1) it was giving a weird "ODK01" error, and (2) it's not actually important to demoing the problem
    console.log(`[wait-before-update]`, 'UPDATE complete.')

    const thingsFinal = await db.query(sql`SELECT * FROM some_thing`);
    console.log(`[wait-before-update]`, 'SELECT 2 complete.')
    const finalCounter = thingsFinal.rows[0].counter;
    return { wait:'before-update', original:originalCounter, final:finalCounter, expected:expectedFinalCounter };
  }));

  service.post('/tx-clash/wait-after-update', endpoint(async ({ Configs, db }, { auth }) => {
    const thingsOriginal = await db.query(sql`SELECT * FROM some_thing`); // suffix FOR UPDATE if we want this to work
    console.log(`[wait-after-update]`, 'SELECT 1 complete.')
    const originalCounter = thingsOriginal.rows[0].counter;

    const expectedFinalCounter = originalCounter + 3;
    await db.query(sql`UPDATE some_thing SET counter=counter+3`);
    console.log(`[wait-after-update]`, 'UPDATE complete.')

    await sleep('wait-after-update', 10000);

    const thingsFinal = await db.query(sql`SELECT * FROM some_thing`);
    console.log(`[wait-after-update]`, 'SELECT 2 complete.')

    const finalCounter = thingsFinal.rows[0].counter;
    return { wait:'after-update', original:originalCounter, final:finalCounter, expected:expectedFinalCounter };
  }));
