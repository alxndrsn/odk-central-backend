...
function getPostLogoutRedirectUri() {
  return `${config.get('default.env.domain')}/`;
}
...
async function initClient() {
  ...
  log('Initialising OIDC client...');

  try {
    assertHasAll('config keys', Object.keys(oidcConfig), ['issuerUrl', 'clientId', 'clientSecret']);

    const { issuerUrl } = oidcConfig;
    log('Attempting to initialise issuer with:', issuerUrl);
    const issuer = await Issuer.discover(issuerUrl);
    log('Initialised issuer:', issuer.issuer, issuer.metadata);
    ...
    if(!scopes_supported) throw new Error(`scopes_supported was not provided in issuer metadata.`);
    ...
    // see: https://openid.net/specs/openid-connect-discovery-1_0.html#ProviderMetadata
    if(!claims_supported) throw new Error(`claims_supported was not provided in issuer metadata.`);
    ...
    const post_logout_redirect_uris = end_session_endpoint ? [getPostLogoutRedirectUri()] : [];
    ...
    log('OIDC client initialised OK.');
    log('client:', client);
