...
function streamBlobs(s3, inStream, remapProps) {
  return PartialPipe.of(inStream, new Transform({
    objectMode: true,
    transform(providedBlob, _, done) {
      const blob = !remapProps ? providedBlob : {
        s3_status: providedBlob[remapProps.s3_status],
        sha: providedBlob[remapProps.sha],
        md5: providedBlob[remapProps.md5],
      };

      if (blob.s3_status === 'uploaded') return s3.getContentFor(blob).then(done, done);
      else done();
    },
  }));
}
