...
const queryFuncs = (db, obj) => {
  ...
  obj.run = (s) => /*console.log('db.run()', s) ||*/ db.query(s).then(always(true));
  obj.run.map = () => () => true;
  ...
  obj.all = async (s) => {
    const doLog = s.sql.includes('COALESCE');
    if(doLog) console.log('db.all()', s);
    const res = await db.any(s);
    if(doLog) console.log('db.all()', s, '->', res);
    return res;
  };
  obj.all.map = (f) => (xs) => {
  ...
  obj.stream = (s) => console.log('db.stream()', s) || new Promise((resolve) => db.stream(s, resolve));
