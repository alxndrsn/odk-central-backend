...
const queryFuncs = (db, obj) => {
  ...
  obj.run = (s) => console.log('executing statement:', s, new Error().stack.split('\n').slice(1).join('\n').trim()) || db.query(s).then(always(true));
  obj.run.map = () => () => true;

  obj.one = (s) => console.log('executing statement:', s, new Error().stack.split('\n').slice(1).join('\n').trim()) || db.one(s);
  obj.one.map = (f) => (x) => f(x);
  obj.maybeOne = (s) => console.log('executing statement:', s, new Error().stack.split('\n').slice(1).join('\n').trim()) || db.maybeOne(s).then(Option.of);
  obj.maybeOne.map = (f) => (x) => x.map(f);
  obj.oneFirst = (s) => console.log('executing statement:', s, new Error().stack.split('\n').slice(1).join('\n').trim()) || db.oneFirst(s);
  obj.oneFirst.map = (f) => (x) => f(x);
  obj.maybeOneFirst = (s) => console.log('executing statement:', s, new Error().stack.split('\n').slice(1).join('\n').trim()) || db.maybeOneFirst(s).then(Option.of);

  obj.all = (s) => console.log('executing statement:', s, new Error().stack.split('\n').slice(1).join('\n').trim()) || db.any(s);
  obj.all.map = (f) => (xs) => {
...
const postgresErrorToProblem = (x) => {
  console.log('postgresErrorToProblem()', x);
