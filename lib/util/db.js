...
const unjoiner = (...frames) => {
  ...
  const unjoin = (row) => {
    ...
    for (const k of Object.keys(bag)) bag[k] = console.log('unjoin()', { k, bag, constructors }) || constructors[k](bag[k]);
...
const queryFuncs = (db, obj) => {
  ...
  obj.all = async (s) => {
    console.log('db.any()', s);
    const res = await db.any(s);
    console.log('db.any()', s, '->', `${res.length} row(s)`, '::', res);
    return res;
  };
  obj.all.map = (f) => (xs) => {
