...
const abortableRequestContext = require('./abortable-request-context');
...
const queryFuncs = (db, obj) => {
  ...
  obj.stream = (s) => new Promise((resolve, reject1) => {
    const abortSignal = null; //abortableRequestContext.getStore();

    db.stream(s, stream => {
      if (abortSignal?.aborted) {
        stream.destroy(new Error('The request was aborted elsewhere before the stream started.'));
        return;
      }
      ...
      try {
        ...
        const handleRequestAborted = () => {
          console.log('handleRequestAborted()', 'destroying stream...');
          stream.destroy(new Error('The request was aborted elsewhere while the stream was in progress.'));
        };
        abortSignal?.addEventListener('abort', handleRequestAborted, { once: true });
        ...
        stream.on('close', () => {
          abortSignal?.removeEventListener('abort', handleRequestAborted);
