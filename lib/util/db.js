...
const unjoiner = (...frames) => {
  ...
  const unjoin = (row, supplied) => {
    const bag = supplied ? clone(supplied) : {};
  ...
  unjoin.with = (supplied) => (row) => Object.assign(unjoin(row, supplied));
  return unjoin;
};
...
const queryFuncs = (db, obj) => {
  ...
  obj.one = (s) => console.log('db.one()', s) || db.one(s);
  obj.one.map = (f) => (x) => f(x);
  obj.maybeOne = (s) => console.log('db.maybeOne()', s) || db.maybeOne(s).then(Option.of);
  obj.maybeOne.map = (f) => (x) => x.map(f);
  obj.oneFirst = (s) => console.log('db.oneFirst()', s) || db.oneFirst(s);
  obj.oneFirst.map = (f) => (x) => f(x);
  obj.maybeOneFirst = (s) => console.log('db.maybeOneFirst()', s) || db.maybeOneFirst(s).then(Option.of);

  obj.all = (s) => console.log('db.all()', s) || db.any(s);
  obj.all.map = (f) => (xs) => {
    const result = new Array(xs.length);
    for (let i = 0; i < xs.length; i += 1) result[i] = f(xs[i]);
    return result;
  };
  obj.allFirst = (s) => console.log('db.allFirst()', s) || db.anyFirst(s);

  obj.stream = (s) => console.log('db.stream()', s) || new Promise((resolve, reject1) => {
    db.stream(s, stream => {
      try {
