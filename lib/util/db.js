...
const queryFuncs = (db, obj) => {
  ...
  obj.run = (s) => console.log('executing statement:', s) || db.query(s).then(always(true));
  obj.run.map = () => () => true;

  obj.one = (s) => console.log('executing statement:', s) || db.one(s);
  obj.one.map = (f) => (x) => f(x);
  obj.maybeOne = (s) => console.log('executing statement:', s) || db.maybeOne(s).then(Option.of);
  obj.maybeOne.map = (f) => (x) => x.map(f);
  obj.oneFirst = (s) => console.log('executing statement:', s) || db.oneFirst(s);
  obj.oneFirst.map = (f) => (x) => f(x);
  obj.maybeOneFirst = (s) => console.log('executing statement:', s) || db.maybeOneFirst(s).then(Option.of);

  obj.all = (s) => console.log('executing statement:', s) || db.any(s);
  obj.all.map = (f) => (xs) => {
