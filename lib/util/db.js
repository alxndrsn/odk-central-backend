...
const queryFuncs = (db, obj) => {
  ...
  obj.one = (s) => console.error('one()', 's:', s) || db.one(s);
  obj.one.map = (f) => (x) => f(x);
  obj.maybeOne = (s) => console.error('maybeOne()', 's:', s) || db.maybeOne(s).then(Option.of);
  obj.maybeOne.map = (f) => (x) => x.map(f);
  obj.oneFirst = (s) => console.error('oneFirst()', 's:', s) || db.oneFirst(s);
  obj.oneFirst.map = (f) => (x) => f(x);
  obj.maybeOneFirst = (s) => console.error('maybeOneFirst()', 's:', s) || db.maybeOneFirst(s).then(Option.of);

  obj.all = (s) => console.log('all()', 's:', s) || db.any(s);
  obj.all.map = (f) => (xs) => {
