...
const unjoiner = (...frames) => {
  ...
  const unjoin = (row, bag={}) => {
    console.log('unjoin()', { row, bag });
  ...
  unjoin.with = (supplied) => (row) => console.log('unjoin.with()', { row, supplied }) || Object.assign(unjoin(row, supplied));
  return unjoin;
};
const partialUnjoiner = (...frames) => {
  const fields = [];
  const unmap = {};
  const unprefix = {};
  const constructors = {};

  for (const { frame: MaybeInstance, fields: props } of frames) {
    const isOption = MaybeInstance instanceof Option;
    const Instance = isOption ? MaybeInstance.get() : MaybeInstance;
    for (const fieldName of props) {
      const field = Instance.fields.find(f => f === fieldName || f.name === fieldName);
      const sqlname = (Instance.from == null) ? fieldName : `${Instance.from}!${fieldName}`;
      const sqlAlias = sql.identifier([sqlname]);
      if (field.selectSql) {
        fields.push(sql`${field.selectSql(Instance.from)} as ${sqlAlias}`);
  ...
  const unjoin = (row) => {
    ...
    const PrimaryFrame = frames[0].frame;

    // pull the primary out and inflate all extensions.
    const primary = bag[PrimaryFrame.to];
    delete bag[PrimaryFrame.to];
    for (const k of Object.keys(bag)) bag[k] = constructors[k](bag[k]);
    return new PrimaryFrame(primary, bag);
  };
  ...
  if (error.code === '22003') {
    ...
    if (match02 != null) {
      ...
      return reject(Problem.user.uniquenessViolation({
        ...
        values: [ projectId, allxmlFormId, version ],
        table: 'forms'
      }));
    }
...
module.exports = {
  ...
  unjoiner, partialUnjoiner, extender, sqlEquals, page, queryFuncs,
