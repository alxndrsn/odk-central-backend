...
const queryFuncs = (db, obj) => {
  ...
  obj.stream = (s) => {
    ...
    return new Promise((resolve, reject) => db.stream(s, stream => {
      const id = 'stream:' + ++lastId;
      console.log('@@@ db.stream', id, 'created', stack);
      stream.on('end',     ()  => console.log('@@@ db.stream', id, '[end]', stack));
      stream.on('close',   ()  => console.log('@@@ db.stream', id, '[close]', stack));
      stream.on('destroy', err => console.log('@@@ db.stream', id, '[destroy] err:', err, stack));
      stream.on('error',   err => console.log('@@@ db.stream', id, '[error] err:', err, stack));
      resolve(stream);
    }).catch(reject));
  }
