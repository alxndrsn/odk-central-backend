...
class PartialPipe {
  ...
  pipeline(errCb) {
    if (arguments.length !== 1) throw new Error(`incorrect arg count: expected 1, but got ${arguments.length}`);
    if (typeof errCb !== `function`) throw new Error(`errCb should be a function, but got ${typeof errCb}`);
    return pipeline(this.streams, rejectIfError(errCb));
  }
