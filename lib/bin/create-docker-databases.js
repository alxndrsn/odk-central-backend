...
(async () => {
  ...
  await Promise.all(['jubilant', 'jubilant_test'].map(async (database) => {
    ...
    await dbj.query('create extension IF NOT EXISTS citext;');
    await dbj.query('create extension IF NOT EXISTS pg_trgm;');
    await dbj.query('create extension IF NOT EXISTS pgrowlocks;');
    dbj.end();
  }));
