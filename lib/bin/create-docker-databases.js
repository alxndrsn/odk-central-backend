...
const connect = async database => {
  const db = new Client({ host: 'localhost', user: 'postgres', password: 'odktest', database });
  await db.connect();
  return db;
};
...
(async () => {
  ...
  for (const database of ['jubilant', 'jubilant_test']) {
    /* eslint-disable no-await-in-loop */ // TODO switch back to Promise.all()
