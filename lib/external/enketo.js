...
const { parseJson } = require('../formats/json');
...
const _init = (hostname, pathname, port, protocol, apiKey) => {
  const enketoRequest = (apiPath, token, postData) => new Promise((resolve, reject) => {
    ...
    const req = request({ hostname, port, headers, method: 'POST', path, auth }, (res) => {
      ...
      res.on('error', reject);
      res.on('end', () => {
        const maybeBody = parseJson(Buffer.concat(resData));
        if (maybeBody.isEmpty()) return reject(Problem.internal.enketoUnexpectedResponse('invalid JSON'));
        const body = maybeBody.get();
