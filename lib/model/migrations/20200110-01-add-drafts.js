...
module.exports = {
  ...
  up: async (db) => {
    await db.raw(`alter table "form_defs" add column "publishedAt" timestamptz`);
    await db.raw(`create index "form_defs_formid_publishedat_index" on "form_defs" ("formId", "publishedAt")`);
    await db.raw(`
        ...
        `);
    await db.raw(`alter table "forms" add column "draftDefId" integer`);
    await db.raw(`alter table "forms" add constraint "forms_draftdefid_foreign" foreign key ("draftDefId") references "form_defs" ("id")`);
    await db.raw(`
