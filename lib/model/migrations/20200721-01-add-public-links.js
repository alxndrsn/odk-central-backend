...
module.exports = {
  up: async (db) => {
    await db.raw(`create table "public_links" ("actorId" integer, "createdBy" integer not null, "formId" integer not null, "once" boolean, "createdAt" timestamptz)`);
    await db.raw(`alter table "public_links" add constraint "public_links_pkey" primary key ("actorId")`);
    await db.raw(`alter table "public_links" add constraint "public_links_actorid_foreign" foreign key ("actorId") references "actors" ("id")`);
    await db.raw(`alter table "public_links" add constraint "public_links_formid_foreign" foreign key ("formId") references "forms" ("id")`);
    await db.raw(`alter table "public_links" add constraint "public_links_createdby_foreign" foreign key ("createdBy") references "actors" ("id")`);
    await db.raw(`insert into "roles" ("createdAt", "name", "system", "verbs") values (NOW(), $1, $2, $3)`, 'Public Link', 'pub-link', '["form.read", "submission.create"]');
    await db.raw(`select "verbs" from "roles" where "system" = $1`, 'admin');
