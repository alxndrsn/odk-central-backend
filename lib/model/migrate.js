...
const patchRaw = (db) => {
  // db is frozen, so we need to proxy
  return new Proxy(db, {
    get(target, prop, receiver) {
      if (prop !== 'raw') return db[prop];
      return function(sql, ...params) {
        console.log('params:', params);
        console.log('sql:', sql);
        sql = sql.replace(/\$[0-9]+/g, '?');
        console.log('sql:', sql);
        return db[prop].call(db, sql, params);
      };
    },
  });
};

module.exports = { checkMigrations, connect, withDatabase, migrate, patchRaw };
