...
const { pick, without, remove, indexOf, omit } = require('ramda');
...
const __aux = '_____________oxoxo___';
...
class Frame {
  ...
  constructor(props, awx) { // can't name this aux bc lint/shadowing
    Object.assign(this, omit([ __aux ], props));
  ...
  forApi() {
    ...
    outer: for (const k of without([ __aux ], Object.keys(this[__aux]))) {
  ...
  with(data) {
    if (data instanceof Frame) {
      const primary = Object.assign({}, this.withoutAux(), data.withoutAux());

      const cleanFrame = f => (f instanceof Frame ? f.withoutAux() : f);
      const extra = Object.assign({}, this[__aux]);
      for (const k of Object.keys(extra))
        extra[k] = cleanFrame(extra[k]);
      for (const k of without([ __aux ], Object.keys(data[__aux])))
        extra[k] = Object.assign({}, extra[k], cleanFrame(data[__aux][k]));

      return new this.constructor(primary, extra);
    }
    return new this.constructor(Object.assign({}, this.withoutAux(), data), this[__aux]);
  }
  withAux(k, v) {
    const data = Object.assign({}, this[__aux]);
    data[k] = v;
    return new this.constructor(this.withoutAux(), data);
  }
  ...
  withoutAux() {
    return omit([ __aux ], { ...this });
  }

  toJSON() {
    return this.withoutAux();
  }
}
