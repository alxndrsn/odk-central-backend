...
const autogenerated = Symbol('autogenerated');
...
class Frame {
  ...
  static define(...parts) {
    const def = { fields: [], autogenerated: [], readable: [], writable: [], aux: [], embedded: [], to: uuid() };

    let last = null;
    for (const part of parts) {
      if (typeof part === 'function') {
        part(def);
      } else if (part === autogenerated) {
        def.autogenerated.push(last);
    ...
    { /* eslint-disable no-shadow */
      ...
      Frame.insertfields = without(def.autogenerated, def.fields);
      Frame.insertFieldTypes = def.fieldTypes?.filter((_, idx) => !def.autogenerated.includes(def.fields[idx]));
      Frame.insertlist = sql.join(Frame.insertfields.map(f => sql.identifier([f])), sql`,`);
      Frame.insertreturnlist = sql.join(def.autogenerated.map(f => sql.identifier([f])), sql`,`);
...
module.exports = { Frame, table, fieldTypes, from, into, aux, species, embedded, autogenerated, readable, writable, ensureDef };
