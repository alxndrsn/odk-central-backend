...
const streamForExport = (datasetId, properties, options = QueryOptions.none, actorId) => async ({ db }) => {
	console.log('properties:', properties, new Error('i am here')); 
//process.exit(1)

	const rawSql = val => sql`${({ sql: val.toString(), type:Symbol.for('SLONIK_TOKEN_SQL'), values:[] })}`;
  const selectExprs = properties.map(p => {
		const { expr, alias } = (p => {
			if(p instanceof Frame) return { expr: sql`entity_defs.data->>${rawSql(`'${p.name}'`)}`, alias: p.name };
			let { expr, alias } = p;
			if(!expr) expr = sql.identifier(p.name.split('.'));
			if(!alias) alias = p.name;
			return { expr, alias };
		})(p);
			
    return sql`${expr} AS ${sql.identifier([alias])}`;
  });

  const selectClause = selectExprs.length > 0
    ? sql`, ${sql.join(selectExprs, sql`, `)}`
    : sql``;

  const query = sql`
    COPY (
      SELECT entities.uuid AS "__id"
           , entity_defs.label AS "label"
        ${selectClause}
      FROM entity_defs
      INNER JOIN entities ON entities.id = entity_defs."entityId"
      LEFT JOIN actors ON entities."creatorId" = actors.id
      WHERE
        entities."datasetId" = ${rawSql(datasetId)}
        AND ${sqlEquals(options.condition)}
        AND ${odataExcludeDeleted(options.filter, odataToColumnMap)}
        AND entity_defs.current = true
        AND ${odataFilter(options.filter, odataToColumnMap)}
...
console.log(query);
  const stream = client.query(copyTo(query.sql, query.values));
