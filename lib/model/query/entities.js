...
const streamForExport = (datasetId, properties, options = QueryOptions.none, actorId) => async ({ db }) => {
  ...
  const selectExprs = properties.map((p) => {
    ...
    const colAlias = sql.identifier([propName]);

    return sql`entity_defs.data->>${rawSql(`'${propName}'`)} AS ${colAlias}`;
  });
  ...
  const query = sql`
    COPY (
      SELECT 
        entities.uuid AS "SubID",
        entities."createdAt" AS "SubmitDate",
        actors."displayName" AS "SubmitterName",
