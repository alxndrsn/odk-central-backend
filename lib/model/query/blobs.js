...
const _getOnePending = () => async ({ all }) => {
  const plan = await all(sql`
  ...
  const res = await all(sql`
    ...
    RETURNING *
  `);
  if (!res.length) return Option.none();
  if (res.length === 1) return Option.of(new Blob(res[0]));

  // eslint-disable-next-line no-console
  console.log(`
    ###############################################################################################
    #
    # Unexpected result length: ${res.length}
    #
    ###############################################################################################
    #
