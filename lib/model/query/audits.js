...
const log = (actor, action, actee, details) => ({ run, context }) => {
  ...
  const jsonDetails = details == null ? null : JSON.stringify(details);

  return run(sql`
    INSERT INTO audits
      ("actorId", action, "acteeId", details, notes, "loggedAt", processed, failures)
      VALUES (${actorId}, ${action}, ${acteeId}::UUID, ${jsonDetails}, ${notes}, clock_timestamp(), ${processed}, 0)
  `);
};
