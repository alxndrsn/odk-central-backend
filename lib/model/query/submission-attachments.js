...
const { blobContent, streamBlobs } = require('../../util/blob');
...
const getCurrentBlobByIds = (formId, submissionId, name, draft) => ({ maybeOne, s3 }) => maybeOne(sql`
with sa as (${_get(formId, submissionId, name, draft)})
select blobs.* from blobs, sa where blobs.id=sa."blobId"`)
  .then(map(construct(Blob)))
  .then(async maybeBlob => {
    if (isEmpty(maybeBlob)) return maybeBlob;
    const blob = maybeBlob.get();
    const content = await blobContent(s3, blob);
    return { get:() => ({ ...blob, content }) }; // TODO how to instantiate a "maybe"?
  });
