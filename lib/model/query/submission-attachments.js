...
const insert = (def, files) => async ({ Blobs, SubmissionAttachments }) => {
  const expecteds = await SubmissionAttachments.getAllByDefId(def.id);
  const lookup = new Set(expecteds.map((att) => att.name));
  const provided = files.filter((file) => lookup.has(file.fieldname));
  return Promise.all(provided.map(async (file) => {
    const blobId = await Blobs.ensure(Blob.fromBuffer(file.buffer, file.mimetype));
    return SubmissionAttachments.attach(def, file.fieldname, blobId);
  }));
};

const attach = (def, name, blobId) => async ({ maybeOneFirst, query }) => {
  const { rowCount } = await query(sql`
    UPDATE submission_attachments
      SET "blobId"=${blobId}
      WHERE "submissionDefId"=${def.id}
        AND name=${name}
        AND ("blobId"=${blobId} OR "blobId" IS NULL)
  `);
  if(rowCount) throw Problem.user.attachmentConflict();
};
