...
select submission_attachments."submissionDefId", submission_attachments."name", submission_attachments."index", submission_attachments."isClientAudit"
     , CASE WHEN submission_attachments."blobId" = -1 THEN NULL ELSE submission_attachments."blobId" END AS "blobId"
...
const getAllByDefId = (submissionDefId) => ({ all }) =>
  all(sql`select submission_attachments."submissionDefId", submission_attachments."name", submission_attachments."index", submission_attachments."isClientAudit"
               , CASE WHEN submission_attachments."blobId" = -1 THEN NULL ELSE submission_attachments."blobId" END AS "blobId"
            from submission_attachments
    where "submissionDefId"=${submissionDefId} order by name`)
...
const getBySubmissionDefIdAndName = (subDefId, name) => ({ maybeOne }) =>
  maybeOne(sql`select submission_attachments."submissionDefId", submission_attachments."name", submission_attachments."index", submission_attachments."isClientAudit"
                    , CASE WHEN submission_attachments."blobId" = -1 THEN NULL ELSE submission_attachments."blobId" END AS "blobId"
                 from submission_attachments
    where "submissionDefId"=${subDefId} and name=${name}`)
    .then(map(construct(Submission.Attachment)));
...
inner join blobs on blobs.id=submission_attachments."blobId" AND blobs.id !== -1
