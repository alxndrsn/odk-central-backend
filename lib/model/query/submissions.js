  ...
  return one(sql`
    ...
    def AS (
      INSERT INTO submission_defs ("submissionId", xml, "formDefId", "instanceId", "instanceName", "submitterId", "localKey", "encDataAttachmentName", signature, "createdAt", root, current, "deviceId", "userAgent")
        SELECT newSubmission.id AS "submissionId"
             , ${sql.binary(partial.xml)}
             , ${form.def.id}
             , "instanceId"
             , ${partial.def.instanceName}
             , "submitterId"
             , ${partial.def.localKey}
             , ${partial.def.encDataAttachmentName}
             , ${partial.def.signature}
             , "createdAt"
             , true
             , true
             , "deviceId"
    ...
    SELECT newSubmission.id
         ...
         , def.id AS "submissionDefId"
         , def."createdAt" AS "submissionDefCreatedAt"
      FROM newSubmission, def;
  `)
