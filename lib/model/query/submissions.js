...
const createNew = (partial, form, deviceIdIn = null, userAgentIn = null) => ({ one, context }) => {
  ...
  return one(sql`
with ins as (insert into submissions ("formId", "instanceId", "submitterId", "deviceId", draft, "createdAt")
...
def as (
  INSERT INTO submission_defs ("submissionId", xml, "formDefId", "instanceId", "instanceName", "submitterId", "localKey", "encDataAttachmentName", signature, "createdAt", root, current, "deviceId", "userAgent")
    SELECT ins.id AS "submissionId"
         , ${sql.binary(partial.xml)}, ${form.def.id}, "instanceId", ${partial.def.instanceName}, "submitterId", ${partial.def.localKey}, ${partial.def.encDataAttachmentName}, ${partial.def.signature}, "createdAt", true, true, "deviceId", ${userAgent}
      FROM ins
      RETURNING id, "createdAt"
)
select ins.id, ins."createdAt", def.id as "submissionDefId", def."createdAt" AS "submissionDefCreatedAt" from ins, def;`)
    .then(unjoinSubmissionAndDefWith({ actorId, deviceId, form, partial, userAgent }));
};

function unjoinSubmissionAndDefWith({ actorId, deviceId, form, partial, userAgent }) {
  return ({ id, createdAt, submissionDefId, submissionDefCreatedAt }) => // TODO/HACK: reassembling this from bits and bobs.
    new Submission({
      id,
