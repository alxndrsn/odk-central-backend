...
select ins.id, ins."createdAt", def.id as "submissionDefId", def."createdAt" AS "submissionDefCreatedAt" from ins, def;`)
    .then(({ id, createdAt, submissionDefId, submissionDefCreatedAt }) => // TODO/HACK: reassembling this from bits and bobs.
      new Submission({
        id,
...
const createVersion = (partial, deprecated, form, deviceIdIn = null, userAgentIn = null) => ({ one, context }) => {
  ...
  const _unjoiner = unjoiner(Submission, Submission.Def.into('currentVersion')); // FIXME this is the key line containing xml...?
