...
const catchXmlReturns = (caller, supplied) => res => {
  const xmls = Object.values(res).filter(it => it?.includes?.('<'));
  if(xmls.length) {
    console.log('XX123 Illegal XML in return value from ', JSON.stringify({ caller, supplied, res }));
  }
  return res;
};

const debugAfterEffects = caller => res => {
  console.log('XX123 unjoined to:', res);
  return res;
};
...
select ins.*, def.id as "submissionDefId" from ins, def;`)
    .then(catchXmlReturns('createNew()'))
...
const createVersion = (partial, deprecated, form, deviceIdIn = null, userAgentIn = null) => ({ one, context }) => {
  ...
  console.log('_unjoiner:', unjoiner);
  console.log('_unjoiner:', Object.keys(unjoiner));

  // we already do transactions but it just feels nice to have the cte do it all at once.
  return one(sql`
with updatedSubmission as (update submissions set "reviewState"='edited', "updatedAt"=clock_timestamp()
  where id=${deprecated.submissionId} 
  returning * )
, deprecatedDef as (update submission_defs set current=false where "submissionId"=${deprecated.submissionId} returning *)
, newDef as (
...
select
        submissions."id"          AS "submissions!id",
  ...
  FROM (
    SELECT updatedSubmission.*
         , deprecatedDef."userAgent" -- the root submission-def's user-agent?
      FROM updatedSubmission
      JOIN deprecatedDef
        ON updatedSubmission.id = deprecatedDef."submissionId"
           AND root
  ) AS submissions
  JOIN newDef AS submission_defs ON TRUE
`)
    .then(catchXmlReturns('createVersion()', { partial, deprecated, form, deviceId, userAgent, actorId }))
    //.then(_unjoiner)
    .then(res => {// TODO/HACK: reassembling this from bits and bobs.
      const fatalError = msg => { console.log(new Error(msg)); process.exit(1); };
      const compare = (resK, original) => {
        if(res[resK] !== original) throw fatalError(`Value for ${resK} did not match: <${res[resK]}> vs <${original}>`);
      };

      compare('submission_defs!submissionId', deprecated.submissionId);

      return new Submission({
        id:          res['submissions!id'],
        formId:      res['submissions!formId'],
        instanceId:  res['submissions!instanceId'],
        submitterId: res['submissions!submitterId'],
        deviceId:    res['submissions!deviceId'],
        createdAt:   res['submissions!createdAt'],
        updatedAt:   res['submissions!updatedAt'],
        reviewState: res['submissions!reviewState'],
        userAgent:   res['submissions!userAgent'],
        draft:       res['submissions!draft'],
        deletedAt:   res['submissions!deletedAt'],
      }, {
        currentVersion: new Submission.Def({
          id:                    res['submission_defs!id'],
          submissionId:          res['submission_defs!submissionId'],
          formDefId:             res['submission_defs!formDefId'],
          submitterId:           res['submission_defs!submitterId'],
          localKey:              res['submission_defs!localKey'],
          encDataAttachmentName: res['submission_defs!encDataAttachmentName'],
          signature:             res['submission_defs!signature'],
          createdAt:             res['submission_defs!createdAt'],
          instanceName:          res['submission_defs!instanceName'],
          instanceId:            res['submission_defs!instanceId'],
          current:               res['submission_defs!current'],
          xml:                   res['submission_defs!xml'],
          root:                  res['submission_defs!root'],
          deviceId:              res['submission_defs!deviceId'],
          userAgent:             res['submission_defs!userAgent'],
        }),
      });
		})
    .then(debugAfterEffects('createVersion()'));
};
