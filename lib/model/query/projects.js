...
const _getSql =((fields, extend, options, actorId) => console.log('_getSql()', { stack:new Error().stack, fields }) || sql`
...
${(actorId == null) ? sql`` : sql`
  INNER JOIN (
    SELECT id
         , verbs -- can we combine here?
      FROM projects
      INNER JOIN (
        SELECT "acteeId"
             , ARRAY_AGG(DISTINCT verb) AS verbs
          FROM (
            SELECT "acteeId"
                 , JSONB_ARRAY_ELEMENTS_TEXT(role.verbs) AS verb
              FROM assignments
              INNER JOIN (SELECT id, verbs FROM roles WHERE verbs ?& ARRAY['project.read', 'form.list']) AS role ON role.id=assignments."roleId"
