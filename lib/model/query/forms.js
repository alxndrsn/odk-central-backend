...
const getByAuthForOpenRosa = (projectId, auth, options = QueryOptions.none, includeClosish) => ({ all }) => all(sql`
...
where "projectId"=${projectId} ${includeClosish ? sql`` : sql `and state not in ('closing', 'closed')`} and "currentDefId" is not null
