...
def as
  ...
  select nextval(pg_get_serial_sequence('forms', 'id')), sch.id, ${form.xml}, ${def.name}, ${def.hash}, ${def.sha}, ${def.sha256}, ${def.version}, ${def.keyId}, ${form.xls.xlsBlobId || -1}, ${def.draftToken || null}, ${def.enketoId || null}, clock_timestamp()
    from sch
  returning *),
form as
...
${extend|| sql`
  ...
  left outer join (select id, "contentType" as "excelContentType" from blobs WHERE id != -1) as xls
