...
const can = (actor, verbOrVerbs, actee) => ({ oneFirst }) => {
  const acteeId = actee.acteeId || actee;

  const verbs = typeof verbOrVerbs === 'string' ? [verbOrVerbs] : verbOrVerbs;

  if (actor.acteeId === acteeId) {
    // special privileges actors always get on themselves.
    if (verbs.includes('user.read') || verbs.includes('user.update'))
      return resolve(true);

    // all actors except app users can always log themselves out.
    // (public links cannot reach this route.)
    if (verbs.includes('session.end') && actor.type !== 'field_key')
      return resolve(true);
  }
    ...
    SELECT EXISTS (
      SELECT 1
        ...
        WHERE "actorId" = ${actor.id}
          AND roles.verbs ?| ARRAY[${sql.array(verbs, 'text')}]
    )
  `);
};
