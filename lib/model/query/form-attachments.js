        ...
        .then(datasets => {
          ...
          const attachments = expecteds.map((expected) => {
            ...
            return new Form.Attachment(mergeRight({
              ...
              blobId: extant.map((e) => e.blobId).orElse(-1),
...
const update = (_, fa, blobId, datasetId = null) => ({ one }) => one(sql`
  update form_attachments set "blobId"=${blobId || -1}, "datasetId"=${datasetId}, "updatedAt"=clock_timestamp()
...
const _get = (exec, options) =>
  ...
  left outer join (select id, md5 from blobs WHERE id != -1) as blobs on form_attachments."blobId"=blobs.id
  where ${sqlEquals(options.condition)} order by name asc`)
    .then(map(attachment => {
      if (attachment['form_attachments!blobId'] === -1) attachment['form_attachments!blobId'] = null;
      return attachment;
    }))
    .then(map(_unjoinMd5));
