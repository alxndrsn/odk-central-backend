...
const update = (_, fa, blobId, datasetId = null) => ({ one }) => one(sql`
  update form_attachments set "blobId"=${blobId}, "datasetId"=${datasetId}, "updatedAt"=clock_timestamp()
    ...
    returning type, "updatedAt"`)
  .then(({ type, updatedAt }) => ({ formId: fa.formId, blobId, name: fa.name, type, formDefId: fa.formDefId, updatedAt, datasetId }))
  .then(construct(Form.Attachment));
