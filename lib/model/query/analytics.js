...
const getDatasets = () => ({ all }) => all(sql`
  SELECT ds.id
       , ds."projectId"
       , COUNT(entz.id) AS num_entities_total
       , COUNT(entz."createdAt" >= current_date - 45 OR NULL) AS num_entities_recent
       , COUNT(entz."updatedAt" IS NOT NULL          OR NULL) AS num_entities_updated_total
       , COUNT(entz."updatedAt" >= current_date - 45 OR NULL) AS num_entities_updated_recent
       , COUNT(fd.*) AS num_creation_forms
       , COUNT(attchmz.*) AS num_followup_forms
       , MAX(errors.total)              AS num_failed_entities_total
       , MAX(errors.recent)             AS num_failed_entities_recent
       , MAX(updates.total)             AS num_entity_updates_total
       , MAX(updates.recent)            AS num_entity_updates_recent
       , MAX(updates.update_sub_total)  AS num_entity_updates_sub_total
       , MAX(updates.update_sub_recent) AS num_entity_updates_sub_recent
       , MAX(updates.update_api_total)  AS num_entity_updates_api_total
       , MAX(updates.update_api_recent) AS num_entity_updates_api_recent
       , MAX(conflict_stats.conflicts)  AS num_entity_conflicts
       , MAX(conflict_stats.resolved)   AS num_entity_conflicts_resolved
       , MAX(creates.create_sub_total)  AS num_entity_create_sub_total
       , MAX(creates.create_sub_recent) AS num_entity_create_sub_recent
       , MAX(creates.create_api_total)  AS num_entity_create_api_total
       , MAX(creates.create_api_recent) AS num_entity_create_api_recent
       , MAX(bulk_creates.total)        AS num_entity_create_bulk_total
       , MAX(bulk_creates.recent)       AS num_entity_create_bulk_recent
    FROM datasets ds
      LEFT JOIN (
      ...
      LEFT JOIN (
        SELECT dfd."datasetId"
          ...
          GROUP BY (dfd."datasetId", fd."formId")
      ...
      LEFT JOIN (
        SELECT fa."datasetId"
          ...
          GROUP BY (fa."datasetId", fa."formId")
      ) AS attchmz ON attchmz."datasetId" = ds.id
      LEFT JOIN (
        SELECT COUNT(*) AS total
             , COUNT(a."loggedAt" >= current_date - 45 OR NULL) AS recent
             , dfd."datasetId"
          FROM audits AS a
            JOIN submissions       AS s   ON CAST((a.details ->> 'submissionId'::TEXT) AS INTEGER) = s.id
      ...
      LEFT JOIN (
        SELECT COUNT(*) total
             , COUNT(a."loggedAt" >= current_date - 45          OR NULL) recent
             , COUNT(a."details"->'submissionDefId' IS NOT NULL OR NULL) update_sub_total
             , COUNT(a."details"->'submissionDefId' IS NOT NULL AND a."loggedAt" >= current_date - 45 OR NULL) update_sub_recent
             , COUNT(a."details"->'submissionDefId' IS     NULL OR NULL) update_api_total
             , COUNT(a."details"->'submissionDefId' IS     NULL AND a."loggedAt" >= current_date - 45 OR NULL) update_api_recent
             , e."datasetId"
        FROM audits AS a
          JOIN entities AS e ON CAST((a.details ->> 'entityId'::TEXT) AS integer) = e.id
        WHERE a."action" = 'entity.update.version'
        GROUP BY e."datasetId"
      ) AS updates ON ds.id = updates."datasetId"
      LEFT JOIN (
        SELECT COUNT(a."details"->'submissionDefId' IS NOT NULL OR NULL) AS create_sub_total
             , COUNT(a."details"->'submissionDefId' IS NOT NULL AND a."loggedAt" >= current_date - 45 OR NULL) AS create_sub_recent
             , COUNT(a."details"->'submissionDefId' IS     NULL OR NULL) AS create_api_total
             , COUNT(a."details"->'submissionDefId' IS     NULL AND a."loggedAt" >= current_date - 45 OR NULL) AS create_api_recent
             , e."datasetId"
        FROM audits AS a
          JOIN entities AS e ON CAST((a.details ->> 'entityId'::TEXT) AS integer) = e.id
      ...
      LEFT JOIN (
        SELECT COUNT(*) AS total
             ...
             , e."datasetId"
          FROM audits a
            JOIN entity_def_sources AS eds ON CAST((a.details ->> 'sourceId'::TEXT) AS integer) = eds."id"
