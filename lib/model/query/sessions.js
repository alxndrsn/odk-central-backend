...
const create = (actor, expiresAt = aDayFromNow()) => async ({ oneFirst }) => {
  const token = generateToken();
  const csrf = generateToken();
  const expiresAtIso = expiresAt.toISOString();
  const createdAt = await oneFirst(sql`
insert into sessions ("actorId", token, csrf, "createdAt", "expiresAt")
values (${actor.id}, ${generateToken()}, ${generateToken()}, clock_timestamp(), ${expiresAtIso})
returning "createdAt"`);
  return new Session({ actorId: actor.id, token, expiresAt: expiresAtIso, createdAt, csrf });
};
