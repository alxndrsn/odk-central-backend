...
const _insertDatasetDef = (dataset, acteeId, actions) => sql`
  ...
  ds AS (
    SELECT id, 'created' "action" FROM ds_ins
    UNION ALL
    SELECT id, 'updated' "action" FROM datasets WHERE name = ${dataset.name} AND "projectId" = ${dataset.projectId}
  ),
  ds_defs AS (
...
const createPublishedDataset = (dataset, project) => async ({ one, oneFirst, Actees, Datasets }) => {
  ...
  if (existingDataset.isDefined()) {
    const { publishedAt: datasetPublished, id: datasetId } = existingDataset.get();
    if (datasetPublished)
    ...
    const publishedAt = await oneFirst(sql`UPDATE datasets SET "publishedAt" = clock_timestamp() where id = ${datasetId} RETURNING "publishedAt"`);
    return existingDataset.get().with({ publishedAt });
  }

  // Final case: publish a completely new dataset
  const actee = await Actees.provision('dataset', project);
  const dsWithId = new Dataset(await one(insert(dataset.with({ acteeId: actee.id }))));
  // TODO it would be nice if this UPDATE was part of the prior INSERT
  const publishedAt = await oneFirst(sql`UPDATE datasets SET "publishedAt" = "createdAt" where id = ${dsWithId.id} RETURNING "publishedAt"`);
  return dsWithId.with({ publishedAt });
};
...
const createPublishedProperty = (property, dataset) => async ({ maybeOne, run }) => {
  ...
  const result = await run(_createPublishedProperty(property));
  if (result.length === 0)
...
const publishIfExists = (formDefId, publishedAt) => async ({ all, context, maybeOne }) => {
  ...
  WITH properties_update as (
    ...
    RETURNING id, name, "datasetId", "publishedAt"
  ), datasets_update as (
...
const _getFormSpecificProperties = (projectId, xmlFormId, forDraft) => sql`
  WITH form AS (
    SELECT fd.id "formDefId", fd."schemaId"
    FROM forms f
  ...
  ds AS (
    SELECT d.id, d.name, d."publishedAt" IS NULL "isNew"
    FROM datasets d
