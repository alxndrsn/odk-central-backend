...
const createPublishedDataset = (dataset, project) => async ({ one, oneFirst, Actees, Datasets }) => {
  ...
  if (existingDataset.isDefined()) {
    ...
    return dataset.with({ publishedAt });
  }

  // Final case: publish a completely new dataset
  const actee = await Actees.provision('dataset', project);

  const id = await oneFirst(sql`
    INSERT INTO datasets (name, "projectId", "acteeId", "createdAt", "publishedAt")
      VALUES (${dataset.name}, ${dataset.projectId}, ${actee.id}, clock_timestamp(), clock_timestamp())
      RETURNING id
  `);
  return dataset.with({ id });
};
