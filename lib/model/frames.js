...
Form.Attachment = class extends Frame.define(
  ...
  forApi() {
    const blobExists = this.blobId != null && this.blobId !== -1;
    const datasetExists = this.datasetId != null;
    const hash = this.aux.blob?.md5 === '0' ? undefined : this.aux.blob?.md5;
    const data = {
      name: this.name,
      type: this.type,
      hash,
...
Submission.Attachment = class extends Frame.define(
  ...
  forApi() {
    return { name: this.name, exists: (this.blobId != null && this.blobId !== -1) };
  }
};
