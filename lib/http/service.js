...
const abortableRequestContext = require('../util/abortable-request-context');
const { cachePolicy, setCachePolicy } = require('../util/http');
const { PartialPipe } = require('../util/stream');

module.exports = (container, additionalResources) => {
  ...
  service.use((req, res, next) => {
    ...
    abortableRequestContext.run(req.signal, () => {
      console.log('abortableSignal:', abortableRequestContext.getStore());
      next();
    });
  });
  ...
  global.__anonymousEndpoint = anonymousEndpoint;
  ...
  additionalResources?.forEach(initResource => {
    initResource({ service, endpoint, anonymousEndpoint });
  });
