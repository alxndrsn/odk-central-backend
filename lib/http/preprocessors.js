...
const { isBlank, isPresent, without } = require('../util/util');
...
const emptyAuthInjector = async ({ Auth }, context) => context.with({ auth: Auth.by(null) });
...
const authHandler = async ({ Sessions, Users, Auth }, context) => {
  const authBySessionToken = async (token, { throwOnFailure }) => {
    const session = await Sessions.getByBearerToken(token);
    if (session.isDefined()) return context.with({ auth: Auth.by(session.get()) });
    if (throwOnFailure) throw Problem.user.authenticationFailed();
  };
  ...
  if (context.fieldKey.isDefined()) {
    ...
    if (!isValidToken(key)) throw Problem.user.authenticationFailed();

    const maybeSession = await Sessions.getByBearerToken(key);
    if (maybeSession.isEmpty()) throw Problem.user.insufficientRights();
  ...
  // Standard Bearer token auth:
    ...
    return authBySessionToken(authHeader.substring(7), { throwOnFailure: true });

  // Basic Auth, which is allowed under certain circumstances:
  } else if (isPresent(authHeader) && authHeader.startsWith('Basic ')) {
    if (oidc.isEnabled()) throw Problem.user.basicAuthNotSupportedWhenOidcEnabled();
    ...
    if (match == null) throw Problem.user.authenticationFailed();
    const [ , email, password ] = match;

    // actually do our verification.
    // TODO: email existence timing attack on whether bcrypt runs or not.
    const maybeUser = await Users.getByEmail(email);
    if (maybeUser.isEmpty()) throw Problem.user.authenticationFailed();

    const user = maybeUser.get();

    const verified = await verifyPassword(password, user.password);
    if (verified === true) return context.with({ auth: Auth.by(user.actor) });
  ...
  // but if the method is anything but GET we will need to check the CSRF token.
    ...
    const maybeSession = authBySessionToken(decodeURIComponent(token), { throwOnFailure: false });
...
const queryOptionsHandler = async (_, context) => {
  ...
  if (Number.isNaN(options.offset))
    throw Problem.user.invalidDataTypeOfParameter({ field: 'offset', expected: 'integer' });
  if (query.limit != null) options.limit = parseInt(query.limit, 10);
  if (Number.isNaN(options.limit))
    throw Problem.user.invalidDataTypeOfParameter({ field: 'limit', expected: 'integer' });
...
const userAgentHandler = async (_, context) => {
