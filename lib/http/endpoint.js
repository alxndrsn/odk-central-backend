...
const endpointBase = ({ preprocessor = noop, before = noop, resultWriter, errorWriter }) => (container, preprocessors = []) => (resource) => (request, response, next) => {
  ...
  // assuming all goes well, run our result writer to actually write an output.
    .then((result) => { console.log('result returned:', result); resultWriter(result, request, response, next); })
  ...
  //
    .catch((err) => {
      console.log('http/endpoint caught err:', err);
...
const manualEndpoint = endpointBase({
  resultWriter: (result, request, response, next) => {
    console.log('manualEndpoint.resultWriter', result, request, response, next);
  },
  errorWriter: (error, request, response) => {
    console.log('manualEndpoint.errorWriter', request, response, err);
  },
});
