...
const rowStreamToOData = (fields, table, domain, originalUrl, query, inStream, tableCount) => {
  ...
  const parserStream = new Transform({
      ...
      submissionToOData(fields, table, row, options).then((data) => {
        ...
        for (const field of data) {
          ...
          if ((counted >= doOffset) && (counted < (doOffset + doLimit))) {
            const push1 = this.push((counted === doOffset) ? '{"value":[' : ','); // header or fencepost.
            console.log('push1:', push1);
            const push2 = this.push(JSON.stringify(fieldRefined));
    ...
    flush(done) {
      ...
      const push3 = this.push((counted <= doOffset) ? '{"value":[],' : '],'); // open object or close row array.
      console.log('push3:', push3);
...
//  inStream.streams[0].on('error', err => console.log('inStream.streams[0]::error', err));
//  inStream.streams[1].on('error', err => console.log('inStream.streams[1]::error', err));
//  parserStream.on('error', err => console.log('parserStream::error', err));
