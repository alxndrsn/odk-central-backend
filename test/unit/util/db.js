...
describe('util/db', () => {
  ...
  describe('unjoiner', () => {
    ...
    it('unjoin.with() should re-integrate supplied data', () => {
      const unjoin = unjoiner(T, Option.of(U));

      sql`${unjoin.fields}`.should.eql(sql`"frames"."x" as "frames!x","frames"."y" as "frames!y","z" as "z"`);

      const unjoined = unjoin.with({ frame: { x: 99, z: -1 }, extra: { a: 10, b: 20, c: 30 } })({ 'frames!x': 3, 'frames!y': 4, z: 5 });
      unjoined.should.eql(new T({ x: 3, y: 4, z: -1 }));
      unjoined.aux.extra.should.eql(Option.of(new U({ z: 5, a: 10, b: 20, c: 30 })));
    });
  });
