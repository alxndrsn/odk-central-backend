...
describe('util/db', () => {
  ...
  describe.only('autogeneratedUnjoiner()', () => {
    const { autogeneratedUnjoiner } = util;

    const X = Frame.define(table('big_frame'),
      'a', 'b', autogenerated,
      'c', 'd', autogenerated,
      'e',
    );
    const Y = Frame.define(into('extra'),
      'f', autogenerated, 'g',
      'h', autogenerated, 'i',
      'j', autogenerated,
    );

    const _unjoiner = autogeneratedUnjoiner(X, Y);
    ...
    it('should unjoin data', () => {
      const unjoined = _unjoiner({ 'big_frame!a': 1, 'big_frame!c': 3, 'big_frame!e': 5, f: 6, h: 8, j: 10 });
      unjoined.should.eql(new X({ a: 1, c: 3, e: 5 }));
      unjoined.aux.extra.should.eql(new Y({ f: 6, h: 8, j: 10 }));
    });

    it('should repopulate supplied data', () => {
      const unjoined = _unjoiner({ b: 2, extra: { i: 9 } }, { 'big_frame!a': 1, 'big_frame!c': 3, 'big_frame!e': 5, f: 6, h: 8, j: 10 });
      unjoined.should.eql(new X({ a: 1, b:2, c: 3, e: 5 }));
      unjoined.aux.extra.should.eql(new Y({ f: 6, h: 8, i: 9, j: 10 }));
    });

    // TODO now use for insert, insertMany(?), ...
    // TODO replace RETURNING * with _unjoiner.fields etc.
  });
