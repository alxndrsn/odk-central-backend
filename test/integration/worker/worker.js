...
describe('worker', () => {
  describe('runner @slow', () => {
    ...
    it.only('should pass the container and event details to the job', testContainerFullTrx(async (container) => {
    ...
    it.only('should run all matched jobs', testContainerFullTrx(async (container) => {
      let count = 0;
      const jobMap = { 'test.event': [
    ...
    it.only('should mark the event as processed after on job completion', testContainerFullTrx(async (container) => {
    ...
    it.only('should log to Sentry if a worker goes wrong', testContainerFullTrx(async (container) => {
      let captured = null;
    ...
    it.only('should still survive and reschedule if Sentry goes wrong', testContainerFullTrx(async (container) => {
    ...
    it.only('should unclaim the event and mark failure in case of failure', testContainerFullTrx(async (container) => {
      const { Audits, Users } = container;
      const alice = (await Users.getByEmail('alice@getodk.org')).get();
    ...
    it.only('should roll back changes in case of error in any job', testContainerFullTrx(async (container) => {
      const { Audits, Users } = container;
      const alice = (await Users.getByEmail('alice@getodk.org')).get();
      await Audits.log(alice.actor, 'submission.attachment.update', alice.actor);
      const event = (await Audits.getLatestByAction('submission.attachment.update')).get();
  ...
  describe('loop', () => {
    ...
    it.only('should run a full loop right away', testContainerFullTrx(async (container) => {
      const { Audits, Users } = container;
      const alice = (await Users.getByEmail('alice@getodk.org')).get();
      await Audits.log(alice.actor, 'submission.attachment.update', alice.actor);
    ...
    it.only('should run two full loops right away', testContainerFullTrx(async (container) => {
      const { Audits, Users } = container;
      const alice = (await Users.getByEmail('alice@getodk.org')).get();
      await Audits.log(alice.actor, 'submission.attachment.update', alice.actor);
      await Audits.log(alice.actor, 'submission.attachment.update', alice.actor);
...
select count(*) from audits where action='submission.attachment.update' and processed is null`)) > 0)
    ...
    it.only('should run two full loops with an idle cycle in between', testContainerFullTrx(async (container) => {
      const { Audits, Users } = container;
      const alice = (await Users.getByEmail('alice@getodk.org')).get();
      await Audits.log(alice.actor, 'submission.attachment.update', alice.actor);
...
select count(*) from audits where action='submission.attachment.update' and processed is null`)) > 0)
    ...
    it.only('should restart if the check fails prequery', testContainerFullTrx(async (container) => {
      const { Audits, Users } = container;
      const alice = (await Users.getByEmail('alice@getodk.org')).get();
      await Audits.log(alice.actor, 'submission.attachment.update', alice.actor);
    ...
    it.only('should restart if the check fails in-query', testContainerFullTrx(async (container) => {
      const { Audits, Users } = container;
      const alice = (await Users.getByEmail('alice@getodk.org')).get();
      await Audits.log(alice.actor, 'submission.attachment.update', alice.actor);
    ...
    it.only('should restart if the process itself fails', testContainerFullTrx(async (container) => {
      const { Audits, Users } = container;
      const alice = (await Users.getByEmail('alice@getodk.org')).get();
      await Audits.log(alice.actor, 'submission.attachment.update', alice.actor);
