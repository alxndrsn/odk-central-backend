...
const { initialize, testContainerRaw, testServiceRaw } = require('../setup');
...
describe.only('database migrations', () => {
  ...
  describe.skip('database migrations', function() {
    ...
    it('should purge deleted forms via migration', testServiceRaw(async (service, container) => {
    ...
    it('should not purge blobs that are still referenced', testServiceRaw(async (service, container) => {
    ...
    it('should purge blobs of deleted forms', testServiceRaw(async (service, container) => {
      // An earlier version of this migration [20220121-02-purge-deleted-forms.js]
      // failed because it tried to purge blobs that were still being used as
      // xlsBlobIds on active form definitons.
      await upToMigration('20220121-01-form-cascade-delete.js');
    ...
    it('should not purge certain form defs that are either published or active drafts', testServiceRaw(async (service, container) => {
    ...
    it('should purge unneeded form draft defs', testServiceRaw(async (service, container) => {
      // 20220209-01-purge-unneeded-drafts.js
      await upToMigration('20220121-02-purge-deleted-forms.js');
      await populateUsers(container);
      await populateForms(container);
  ...
  describe('database migrations: removing default project', function() {
    ...
    it('should put old forms into project', testServiceRaw(async (service, container) => {
    ...
    it('should not make a default project if no forms', testServiceRaw(async (service, container) => {
  ...
  describe('database migrations: intermediate form schema', function() {
    ...
    it('should test migration', testServiceRaw(async (service, container) => {
  ...
  describe('database migrations: 20230123-01-remove-google-backups', function() {
    ...
    it('deletes backups configs', testContainerRaw(async ({ Configs }) => {
    ...
    describe('backup creation token', () => {
      ...
      it('consumes a token', testContainerRaw(async (container) => {
      ...
      it('does not modify other actor data', testServiceRaw(async (service, container) => {
        await populateUsers(container);
    ...
    it('deletes the role that grants backup.verify', testContainerRaw(async ({ Roles }) => {
  ...
  describe.skip('database migrations: 20230324-01-edit-dataset-verbs.js', function () {
    ...
    it('should add dataset/entity read verbs with raw sql', testServiceRaw(async (service, container) => {
  ...
  describe.skip('database migrations from 20230406: altering entities and entity_defs', function () {
    ...
    it('should set entity def creator id and user agent from submission def', testServiceRaw(async (service, container) => {
    ...
    it('should move the entity label to the entity_def table', testServiceRaw(async (service, container) => {
      await upToMigration('20230406-01-add-entity-def-fields.js', false);
      await populateUsers(container);
      await populateForms(container);
  ...
  describe('database migrations from 20230512: adding entity_def_sources table', function () {
    ...
    it('should backfill entityId and entityDefId in audit log', testServiceRaw(async (service, container) => {
    ...
    it.skip('should migrate the submissionDefId source of an entity to new table with event links', testServiceRaw(async (service, container) => {
  ...
  describe('database migrations from 20230802: delete orphan submissions', function test() {
    ...
    it('should delete orphan draft Submissions', testServiceRaw(async (service, container) => {
  ...
  describe.skip('database migration: 20231002-01-add-conflict-details.js', function test() {
    ...
    it('should update dataReceived and baseVersion of existing entity defs', testServiceRaw(async (service, container) => {
  ...
  testMigration('20240215-01-entity-delete-verb.js', () => {
    it('should add entity.delete verb to correct roles', testServiceRaw(async (service) => {
  ...
  testMigration('20240215-02-dedupe-verbs.js', () => {
    it('should remove duplicate submission.update verb', testServiceRaw(async (service) => {
      const verbsByRole = async () => {
        const { body: roles } = await service.get('/v1/roles').expect(200);
        const bySystem = {};
        for (const role of roles) bySystem[role.system] = role.verbs;
    ...
    it('should result in unique verbs for all roles', testServiceRaw(async (service) => {
      await up();
      const { body: roles } = await service.get('/v1/roles').expect(200);
  ...
  testMigration('20240914-02-remove-orphaned-client-audits.js', () => {
    it('should remove orphaned client audits', testServiceRaw(async (service, container) => {
      await populateUsers(container);
      await populateForms(container);
    ...
    testMigration('20241010-01-schedule-entity-form-upgrade.js', () => {
      it('should schedule entity forms with spec version 2023.1.0 for upgrade to 2024.1.0', testServiceRaw(async (service, container) => {
        await populateUsers(container);
        await populateForms(container);
